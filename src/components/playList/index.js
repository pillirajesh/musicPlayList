...
class PlayList extends Component {
  state = {newTrack: initialTracksList, songInput: ''}

  deleteTrack = selectedTrack => {
    const {newTrack} = this.state
    const filteredTrack = newTrack.filter(each => each.id !== selectedTrack)
    this.setState({newTrack: filteredTrack})
  }
    ...
    return (
      <div className="app-container">
        <div className="top-container">
          <div>
            <h1 className="top-heading">Ed Sheeran</h1>
            <p className="top-paragraph">Singer</p>
          </div>
        </div>
              ...
              {track.map(eachTrack => (
                <li key={eachTrack.id} className="list-items">
                  <div className="genre-container">
                    ...
                    />
                    <div>
                      <p className="song-name">{eachTrack.name}</p>
                      <p className="genre-name">{eachTrack.genre}</p>
                    </div>
                  </div>
                  <div>
                    <div className="delete-container">
                      ...
                      <button
                        ...
                        type="button"
                        data-testid="delete"
                        onClick={() => this.deleteTrack(eachTrack.id)}
                      >
